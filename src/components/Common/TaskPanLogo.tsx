...
import appIconImg from '../../assets/images/taskpan_app_icon_1787588819205.jpg';
  ...
  return (
    <div className={`relative inline-flex items-center justify-center flex-shrink-0 ${currentSizeClass} ${className}`}>
      {useImage && !imgError ? (
        <img
          src={appIconImg}
          alt="TaskPan 3D App Icon"
          referrerPolicy="no-referrer"
          onError={() => setImgError(true)}
          className="w-full h-full object-cover rounded-2xl shadow-md transition-transform duration-300 hover:scale-105"
        />
      ) : (
        /* 3D Clay Gradient Squircle SVG Fallback */
        <svg
          viewBox="0 0 120 120"
          fill="none"
